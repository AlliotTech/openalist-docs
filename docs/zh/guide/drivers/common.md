...
- ADDRESS: 你的 AList 地址，必须加上协议头，后面不能跟 `/`。如 `https://pan.iots.vip`；
...
@tab 通用二进制
也可以使用另一台机器进行代理，在 https://github.com/AlliotTech/openalist-proxy/releases 下载程序并通过 `./alist-proxy -help` 查看使用方法。
